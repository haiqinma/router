...
| 方向 | V1 现状 | V2 目标 |
| --- | --- | --- |
| 路由解释 | 主要依赖日志和分散页面排查 | 每次选路保存候选池、过滤原因、最终渠道、失败切换原因 |
| 模型健康 | 已展示平台服务状态，但样本和解释仍需增强 | 模型、渠道、端点三层健康状态可追溯到最近请求和测试事实 |
| 采购成本 | 已有采购批次、消耗记录、报表口径 | 区分已落地口径、估算口径、未配置成本、成本归因失败重试 |
...
| 阶段 | 目标 | 不做 |
| --- | --- | --- |
| V2.1 | 成本归因稳定，失败有 pending / retry 状态 | 不自动改价 |
