...
本文档记录 V2 的最终收口结果。当前已落地基线见 [路由架构 V1](./路由架构V1.md)。
...
7. 日志已增加版本化 `billing_decision`，统一记录计价来源、用量来源、结算方式、倍率、金额和定价决策。

V2 已收口完成，以下增强进入 V3：

1. 渠道缓存构建阶段的禁用、熔断等排除事实结构化记录。
2. 路由和财务异常的更细运营建议与自动处置。
3. `BillingDecision` 的更高版本和更细粒度成本归因解释。
...
| 方向 | V1 现状 | V2 目标 |
| --- | --- | --- |
| 路由解释 | 已保存过滤后候选渠道、初始/最终渠道、过滤原因和失败尝试 | 更细的缓存构建排除事实和自动处置建议转入 V3 |
| 模型健康 | 已区分真实调用、渠道检测和无信号 | 模型、渠道、端点三层健康状态可追溯到最近请求和测试事实 |
| 采购成本 | 已形成六种请求级归因状态，重试闭环和人工重试入口已落地 | 更细的失败归因建议转入 V3 |
| 计价决策 | 已接入官方锚点、采购成本底线和结构化 `BillingDecision` | 更高版本的成本解释和调价建议转入 V3 |
